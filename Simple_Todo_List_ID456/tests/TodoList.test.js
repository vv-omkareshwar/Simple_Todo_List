// Mock React implementation
const React = {
  useState: (initialState) => {
    let state = initialState;
    const setState = (newState) => {
      state = typeof newState === 'function' ? newState(state) : newState;
    };
    return [state, setState];
  },
};

// TodoList component implementation
const TodoList = () => {
  const [todos, setTodos] = React.useState([]);
  const [inputValue, setInputValue] = React.useState('');

  const addTodo = () => {
    const todoText = inputValue.trim();
    if (todoText !== '') {
      setTodos([...todos, todoText]);
      setInputValue('');
    }
  };

  const removeTodo = (index) => {
    const newTodos = todos.filter((_, i) => i !== index);
    setTodos(newTodos);
  };

  const handleKeyPress = (event) => {
    if (event.key === 'Enter') {
      addTodo();
    }
  };

  return {
    todos,
    inputValue,
    addTodo,
    removeTodo,
    handleKeyPress,
    setInputValue,
  };
};

// Test suite
describe('TodoList Component', () => {
  let todoList;

  beforeEach(() => {
    todoList = TodoList();
  });

  test('initial state', () => {
    expect(todoList.todos).toEqual([]);
    expect(todoList.inputValue).toBe('');
  });

  test('addTodo - adds a new todo', () => {
    todoList.setInputValue('New Todo');
    todoList.addTodo();
    expect(todoList.todos).toEqual(['New Todo']);
    expect(todoList.inputValue).toBe('');
  });

  test('addTodo - does not add empty todo', () => {
    todoList.setInputValue('   ');
    todoList.addTodo();
    expect(todoList.todos).toEqual([]);
  });

  test('removeTodo - removes a todo', () => {
    todoList.setInputValue('Todo 1');
    todoList.addTodo();
    todoList.setInputValue('Todo 2');
    todoList.addTodo();
    todoList.removeTodo(0);
    expect(todoList.todos).toEqual(['Todo 2']);
  });

  test('handleKeyPress - adds todo on Enter key', () => {
    todoList.setInputValue('New Todo');
    todoList.handleKeyPress({ key: 'Enter' });
    expect(todoList.todos).toEqual(['New Todo']);
  });

  test('handleKeyPress - does not add todo on other keys', () => {
    todoList.setInputValue('New Todo');
    todoList.handleKeyPress({ key: 'Space' });
    expect(todoList.todos).toEqual([]);
  });
});

// Test runner implementation
function describe(name, fn) {
  console.log(`\nTest Suite: ${name}`);
  fn();
}

function test(name, fn) {
  console.log(`\nTest: ${name}`);
  fn();
}

function expect(actual) {
  return {
    toEqual: (expected) => {
      const result = JSON.stringify(actual) === JSON.stringify(expected);
      console.log(`Expected: ${JSON.stringify(expected)}`);
      console.log(`Actual: ${JSON.stringify(actual)}`);
      console.log(`Result: ${result ? 'PASS' : 'FAIL'}`);
    },
    toBe: (expected) => {
      const result = actual === expected;
      console.log(`Expected: ${expected}`);
      console.log(`Actual: ${actual}`);
      console.log(`Result: ${result ? 'PASS' : 'FAIL'}`);
    },
  };
}

function beforeEach(fn) {
  global.beforeEachFn = fn;
}

// Run the tests
describe('TodoList Component', () => {
  let todoList;

  beforeEach(() => {
    todoList = TodoList();
  });

  test('initial state', () => {
    global.beforeEachFn();
    expect(todoList.todos).toEqual([]);
    expect(todoList.inputValue).toBe('');
  });

  test('addTodo - adds a new todo', () => {
    global.beforeEachFn();
    todoList.setInputValue('New Todo');
    todoList.addTodo();
    expect(todoList.todos).toEqual(['New Todo']);
    expect(todoList.inputValue).toBe('');
  });

  test('addTodo - does not add empty todo', () => {
    global.beforeEachFn();
    todoList.setInputValue('   ');
    todoList.addTodo();
    expect(todoList.todos).toEqual([]);
  });

  test('removeTodo - removes a todo', () => {
    global.beforeEachFn();
    todoList.setInputValue('Todo 1');
    todoList.addTodo();
    todoList.setInputValue('Todo 2');
    todoList.addTodo();
    todoList.removeTodo(0);
    expect(todoList.todos).toEqual(['Todo 2']);
  });

  test('handleKeyPress - adds todo on Enter key', () => {
    global.beforeEachFn();
    todoList.setInputValue('New Todo');
    todoList.handleKeyPress({ key: 'Enter' });
    expect(todoList.todos).toEqual(['New Todo']);
  });

  test('handleKeyPress - does not add todo on other keys', () => {
    global.beforeEachFn();
    todoList.setInputValue('New Todo');
    todoList.handleKeyPress({ key: 'Space' });
    expect(todoList.todos).toEqual([]);
  });
});

console.log('\nAll tests completed.');