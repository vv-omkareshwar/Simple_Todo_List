...
function taskStufs(e: Event): void {
  ...
  if (target.classList.contains("delete-item")) {
    ...
    let tmpList = loadFromLocalStorage<Task[]>("allTasks") || [];

    const taskElement = target.parentElement?.children[0] as HTMLElement;
    if (taskElement.classList.contains("line-throught")) {
      for (let i = 0; i < tmpList.length; i++) {
        if (tmpList[i].text == taskElement.innerText) {
          tmpList[i].check = false;
          saveToLocalStorage("allTasks", tmpList);
          break;
        }
      }

      taskElement.classList.remove("line-throught");
    } else {
      for (let i = 0; i < tmpList.length; i++) {
        if (tmpList[i].text == taskElement.innerText) {
      ...
      taskElement.classList.add("line-throught");
    }

    // adding inner task with button
  } else if (target.classList.contains("inner-check")) {
    let tmp = loadFromLocalStorage<InnerTask[]>("innerTasks") || [];
    ...
    if (taskElement.classList.contains("line-throught")) {
      tmp.forEach(function (item) {
        if (item.mainText == taskElement.innerText && item.upperTaskText == upperText) {
          item.check = false;
          saveToLocalStorage("innerTasks", tmp);
        }
      });
      taskElement.classList.remove("line-throught");
    } else {
...
function saveLS(text: string, index: number): void {
  ...
  if (index == -1) {
    ...
    const tmp = loadFromLocalStorage<Task[]>("allTasks") || [];
    ...
    if (c !== undefined) {
      tmp.splice(c, 1);
      saveToLocalStorage("allTasks", tmp);
    }

    // delete inner tasks whose upper text that
    const tmp2 = loadFromLocalStorage<InnerTask[]>("innerTasks") || [];
...
function saveInnerTasks(item: string | InnerTask, index: number, upperText: string = ""): void {
  ...
  if (index == -1) {
    ...
    const tmp = loadFromLocalStorage<InnerTask[]>("innerTasks") || [];
...
export { eventListeners, createNewTask, addNewItem, addInnerTask, taskStufs, deleteAll, loadLS, loadInnerTasks, saveLS, saveInnerTasks };