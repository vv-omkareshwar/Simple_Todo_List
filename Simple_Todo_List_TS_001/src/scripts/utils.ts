...
function addInnerTask(upperText: string, mainText: string, check: boolean): void {
    ...
    if (tempList) {
        const innerTaskList = tempList.lastElementChild?.firstElementChild?.firstElementChild as HTMLUListElement;
        innerTaskList.insertBefore(
            tempLi,
            innerTaskList.lastElementChild
        );
    }
}
